...
<template>
  <div v-if="editor" class="border border-base-300 rounded-lg bg-base-100">
    <div class="flex flex-wrap items-center gap-0.5 p-1 border-b border-base-300 bg-base-200 rounded-t-lg">
      <button type="button" class="btn btn-xs btn-ghost font-bold"
        :class="{ 'btn-active': editor.isActive('bold') }"
        @click="editor.chain().focus().toggleBold().run()" title="Gras (Ctrl+B)">G</button>
      <button type="button" class="btn btn-xs btn-ghost italic"
        :class="{ 'btn-active': editor.isActive('italic') }"
        @click="editor.chain().focus().toggleItalic().run()" title="Italique (Ctrl+I)">I</button>
      <button type="button" class="btn btn-xs btn-ghost underline"
        :class="{ 'btn-active': editor.isActive('underline') }"
        @click="editor.chain().focus().toggleUnderline().run()" title="Souligné (Ctrl+U)">U</button>
      <button type="button" class="btn btn-xs btn-ghost line-through"
        :class="{ 'btn-active': editor.isActive('strike') }"
        @click="editor.chain().focus().toggleStrike().run()" title="Barré">S</button>

      <span class="w-px h-5 bg-base-300 mx-1"></span>

      <button type="button" class="btn btn-xs btn-ghost"
        :class="{ 'btn-active': editor.isActive('link') }"
        @click="setLink" title="Lien">
      ...
      <button type="button" class="btn btn-xs btn-ghost"
        ...
        @click="editor.chain().focus().toggleBulletList().run()" title="Liste à puces">
        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><circle cx="4" cy="6" r="1" fill="currentColor"/><circle cx="4" cy="12" r="1" fill="currentColor"/><circle cx="4" cy="18" r="1" fill="currentColor"/></svg>
      </button>
    </div>
    <EditorContent :editor="editor" class="prose prose-sm max-w-none px-3 py-2 min-h-[3.5rem]" />
