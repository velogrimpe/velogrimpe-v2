...
const liaisonDescRefs = reactive<Record<number, { setContent: (h: string) => void } | null>>({})
const ligneDescRefs = reactive<Record<string, { setContent: (h: string) => void } | null>>({})
  ...
  (keys) => {
    ...
    for (const k of keys) {
      ...
      if (k.startsWith('id:')) {
        ...
        ligneMap[k] = { key: k, id: null, nom: row?.ligneNom ?? '', description: '', lien: '' }
      }
    }
...
async function removeLiaison(row: LiaisonRow) {
  if (row.persistedId != null) {
    const nom_prenom = (document.getElementById('nom_prenom') as HTMLInputElement)?.value ?? ''
    const email = (document.getElementById('email') as HTMLInputElement)?.value ?? ''
    try {
      const res = await fetch('/api/delete_bus_liaison.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ liaison_id: row.persistedId, nom_prenom, email }),
      })
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
        alert('Erreur lors de la suppression de la liaison : ' + (err.error || res.status))
        return
      }
    } catch (e) {
...
function onLigneSelect(row: LiaisonRow, item: FormAutocompleteItem | null, value: string) {
  ...
  <div class="flex flex-col gap-6">
    ...
    <div>
      <div class="relative flex items-center mb-3">
        ...
        <span class="px-2 text-primary italic">Liaisons</span>
        <hr class="my-0 grow border-[#2e8b57]" />
      </div>
            ...
            <div class="flex flex-col md:flex-row md:items-end gap-2 flex-wrap">
              ...
              <div class="relative not-prose grow min-w-[12rem]">
                <FormAutocomplete
                  ...
                  @select="(item: FormAutocompleteItem | null) => onArretSelect(row, item)"
                />
              </div>
              ...
              <div class="relative not-prose grow min-w-[12rem]">
                <FormAutocomplete
                  ...
                  @select="(item: FormAutocompleteItem | null, value: string) => onLigneSelect(row, item, value)"
                />
              </div>
            </div>
            <label class="form-control">
              <span class="text-sm font-bold">Détail sur la liaison <i class="opacity-50 font-normal">(optionnel)</i></span>
              <RichTextField
      ...
      <button type="button" class="btn btn-sm btn-outline btn-primary mt-3" @click="addLiaison">
      ...
      <div class="flex flex-col gap-4">
        ...
        >
          <div class="flex items-center gap-2">
            <span class="badge badge-primary badge-sm">{{ ligne.id ? 'Ligne existante' : 'Nouvelle ligne' }}</span>
          </div>
          <label class="form-control">
          ...
          <label class="form-control">
            <span class="text-sm font-bold">Description <i class="opacity-50 font-normal">(optionnel)</i></span>
            <RichTextField
          ...
          <label class="form-control">
            <span class="text-sm font-bold">Lien vers les horaires <i class="opacity-50 font-normal">(optionnel)</i></span>
            <input
              type="url"
              class="input input-primary input-sm"
              v-model="ligne.lien"
