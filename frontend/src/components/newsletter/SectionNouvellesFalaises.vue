...
function addFalaise(
  regionIndex: number,
  falaise: { id: number; name: string; contributor?: string },
) {
  const regions = [...(props.section.regions ?? [])];
  ...
  region.falaises = [
    ...region.falaises,
    {
      id: falaise.id,
      name: falaise.name,
      contributor: falaise.contributor ?? "",
    },
  ];
...
<template>
  <div class="space-y-4">
    ...
    >
      <div>
        <label class="label font-medium">Nom de la région</label>
        <div class="flex gap-2 items-center">
          ...
          <button
            class="btn btn-xs"
            :disabled="ri === 0"
            @click="moveRegion(ri, -1)"
          >
            ↑
          </button>
          <button
            class="btn btn-xs"
            :disabled="ri === (section.regions?.length ?? 0) - 1"
            @click="moveRegion(ri, 1)"
          >
            ↓
          </button>
      ...
      <div>
        <label class="label font-medium">Image d'aperçu (optionnelle)</label>
        <p class="text-xs text-base-content/60 mb-1">
          Affichée en tête de section dans la newsletter, au-dessus de la liste
          des falaises.
        </p>
        <div class="flex gap-2 items-center">
          <input
            ...
            @change="uploadRegionImage(ri, $event)"
          />
          <img
            v-if="region.image"
            :src="region.image"
      ...
      <div>
        <label class="label font-medium">Falaises de la région</label>
        <a
          v-if="region.falaises.length > 0"
          :href="`https://velogrimpe.fr/carte.php?h=${region.falaises.map((f) => f.id).join(',')}`"
          target="_blank"
          rel="noopener"
          class="link link-primary text-xs block mb-2"
        >
          Voir ces falaises sur la carte ↗
        </a>
        <div class="space-y-2">
          <div
            v-for="(f, fi) in region.falaises"
            :key="fi"
            class="flex gap-2 items-center bg-primary/10 rounded px-2 py-1"
          >
