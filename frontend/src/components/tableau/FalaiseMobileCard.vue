...
<template>
  <a
    ...
    class="text-base-content hover:no-underline font-normal"
  >
    <div class="flex flex-col rounded-lg shadow-xl bg-base-100 p-2 text-sm">
      <div class="flex flex-row justify-between gap-1">
        <h3 class="text-xl font-bold text-primary hover:underline">
          {{ common.falaise_nom }}
          <span v-if="common.falaise_fermee" class="text-error font-normal">Falaise Interdite</span>
        </h3>
      ...
      <div class="w-full flex flex-row items-center justify-between gap-2">
        <div class="flex flex-col items-start justify-start flex-grow">
          ...
          <div>
            ...
            <span>de {{ common.falaise_cotmin }} à {{ common.falaise_cotmax }}</span>
          </div>
          ...
          <div>
            ...
            <span v-if="common.falaise_maa > 0">{{ formatTime(common.falaise_maa) }}</span>
        ...
        <ul class="list-disc list-inside">
          <li v-for="(row, idx) in falaise" :key="idx">
            <template v-if="row.train_temps > 0">
              ...
              ({{ formatTime(row.train_temps) }}, <span title="D=Direct / C=Correspondances">{{ formatCorrespondances(row) }}</span>) +
            </template>
