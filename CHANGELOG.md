...
- Formulaire d'arrêt de bus (`ajout/ajout_bus.php`) : les falaises liées à l'arrêt s'affichent maintenant en liste sous la carte, chacune avec un « × » pour la délier, et un bouton « + Lier une falaise » ouvre un autocomplete de recherche ; un ajout par ce biais recadre la carte sur l'arrêt et l'ensemble des falaises liées (zoom maximum 16). Lier une falaise ne suppose donc plus de retrouver son marqueur sur la carte (ni qu'elle en ait un). Les deux voies restent synchronisées : le script carte de la page reste seul détenteur de l'état (champ caché `#arret_falaise_ids`, ce qui part au POST) et prévient le composant Vue `FalaisesLiees.vue` par l'événement `velogrimpe:bus-falaises-changed` ; côté API, rien ne change. Tests : `tests/e2e/ajout-bus-falaises.spec.ts`.
